...
GET_QUANTITY_FOR_COIN_SELL = "SELECT cantidad_from FROM movements WHERE moneda_from = ?"
GET_QUANTITY_FOR_COIN_BUY = "SELECT cantidad_to FROM movements WHERE moneda_to = ?"
...
def check_balance_for_currency(coin_from):
    ...
    cur = conn.cursor()
    balance_sell = cur.execute(GET_QUANTITY_FOR_COIN_SELL, (coin_from, ))
    for q in balance_sell:
        total_sell += q[0]

    cur2 = conn.cursor()
    balance_buy = cur2.execute(GET_QUANTITY_FOR_COIN_BUY, (coin_from, ))
    for q in balance_buy:
        total_buy += q[0]
